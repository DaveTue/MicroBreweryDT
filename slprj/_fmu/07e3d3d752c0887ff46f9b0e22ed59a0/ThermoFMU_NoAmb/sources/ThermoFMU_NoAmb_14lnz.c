/* Linearization */
#include "ThermoFMU_NoAmb_model.h"
#if defined(__cplusplus)
extern "C" {
#endif
const char *ThermoFMU_NoAmb_linear_model_frame()
{
  return "model linearized_model \"ThermoFMU_NoAmb\"\n"
  "  parameter Integer n = 1 \"number of states\";\n"
  "  parameter Integer m = 3 \"number of inputs\";\n"
  "  parameter Integer p = 2 \"number of outputs\";\n"
  "  parameter Real x0[n] = %s;\n"
  "  parameter Real u0[m] = %s;\n"
  "\n"
  "  parameter Real A[n, n] =\n\t[%s];\n\n"
  "  parameter Real B[n, m] =\n\t[%s];\n\n"
  "  parameter Real C[p, n] =\n\t[%s];\n\n"
  "  parameter Real D[p, m] =\n\t[%s];\n\n"
  "\n"
  "  Real x[n](start=x0);\n"
  "  input Real u[m](start=u0);\n"
  "  output Real y[p];\n"
  "\n"
  "  Real 'x_T' = x[1];\n"
  "  Real 'u_cg_rate' = u[1];\n"
  "  Real 'u_pump_state' = u[2];\n"
  "  Real 'u_sg' = u[3];\n"
  "  Real 'y_Q_beer' = y[1];\n"
  "  Real 'y_Temperature' = y[2];\n"
  "equation\n"
  "  der(x) = A * x + B * u;\n"
  "  y = C * x + D * u;\n"
  "end linearized_model;\n";
}
const char *ThermoFMU_NoAmb_linear_model_datarecovery_frame()
{
  return "model linearized_model \"ThermoFMU_NoAmb\"\n"
  "  parameter Integer n = 1 \"number of states\";\n"
  "  parameter Integer m = 3 \"number of inputs\";\n"
  "  parameter Integer p = 2 \"number of outputs\";\n"
  "  parameter Integer nz = 7 \"data recovery variables\";\n"
  "  parameter Real x0[1] = %s;\n"
  "  parameter Real u0[3] = %s;\n"
  "  parameter Real z0[7] = %s;\n"
  "\n"
  "  parameter Real A[n, n] =\n\t[%s];\n\n"
  "  parameter Real B[n, m] =\n\t[%s];\n\n"
  "  parameter Real C[p, n] =\n\t[%s];\n\n"
  "  parameter Real D[p, m] =\n\t[%s];\n\n"
  "  parameter Real Cz[nz, n] =\n\t[%s];\n\n"
  "  parameter Real Dz[nz, m] =\n\t[%s];\n\n"
  "\n"
  "  Real x[n](start=x0);\n"
  "  input Real u[m](start=u0);\n"
  "  output Real y[p];\n"
  "  output Real z[nz];\n"
  "\n"
  "  Real 'x_T' = x[1];\n"
  "  Real 'u_cg_rate' = u[1];\n"
  "  Real 'u_pump_state' = u[2];\n"
  "  Real 'u_sg' = u[3];\n"
  "  Real 'y_Q_beer' = y[1];\n"
  "  Real 'y_Temperature' = y[2];\n"
  "  Real 'z_Q_beer' = z[1];\n"
  "  Real 'z_Temperature' = z[2];\n"
  "  Real 'z_Vol_meters' = z[3];\n"
  "  Real 'z_cg_rate' = z[4];\n"
  "  Real 'z_density' = z[5];\n"
  "  Real 'z_pump_state' = z[6];\n"
  "  Real 'z_sg' = z[7];\n"
  "equation\n"
  "  der(x) = A * x + B * u;\n"
  "  y = C * x + D * u;\n"
  "  z = Cz * x + Dz * u;\n"
  "end linearized_model;\n";
}
#if defined(__cplusplus)
}
#endif

