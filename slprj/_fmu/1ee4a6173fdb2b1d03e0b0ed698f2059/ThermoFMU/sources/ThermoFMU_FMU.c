...
#define MODEL_IDENTIFIER ThermoFMU
#define MODEL_GUID "{1717b159-3f7f-4632-8787-9d0878ee4531}"
...
#define FMI2_FUNCTION_PREFIX ThermoFMU_
...
extern void ThermoFMU_setupDataStruc(DATA *data, threadData_t *threadData);
#define fmu2_model_interface_setupDataStruc ThermoFMU_setupDataStruc
...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  comp->fmuData->modelData->realParameterData[0].attribute.start = 0.1817;
  comp->fmuData->modelData->realParameterData[1].attribute.start = 0.754;
  comp->fmuData->modelData->realParameterData[2].attribute.start = 1.2552;
  comp->fmuData->modelData->realParameterData[3].attribute.start = 0.0;
  comp->fmuData->modelData->realParameterData[4].attribute.start = 298.15;
  comp->fmuData->modelData->realParameterData[5].attribute.start = 290.15;
  comp->fmuData->modelData->realParameterData[6].attribute.start = 25.0;
  comp->fmuData->modelData->realParameterData[7].attribute.start = 91200.0;
  comp->fmuData->modelData->realParameterData[8].attribute.start = 322.89;
  comp->fmuData->modelData->realParameterData[9].attribute.start = 42.9315;
}
// Set values for all variables that define a start value
OMC_DISABLE_OPT
void setStartValues(ModelInstance *comp) {
...
fmi2Real getReal(ModelInstance* comp, const fmi2ValueReference vr) {
  ...
  if (vr < 19) {
    return comp->fmuData->simulationInfo->realParameter[vr-9];
  }
  if (vr < 21) {
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 9 && (comp->state == model_state_instantiated || comp->state == model_state_initialization_mode)) {
  ...
  if (vr < 19) {
    comp->fmuData->simulationInfo->realParameter[vr-9] = value;
    return fmi2OK;
  }
  if (vr < 21) {
    int ix = realAliasIndexes[vr-19];
