...
#         # Register variables with the FMU interface
#         # Inputs
#         self.register_real_variable("sigma_sensor", "Sensor variance", "input", getter=lambda: self.sigma_sensor, setter=self._set_sigma)
#         self.register_real_variable("x_measure", "Measured value", "input", getter=lambda: self.x_measure, setter=self._set_x_measure)
#         self.register_real_variable("x_cal", "Calculated value", "input", getter=lambda: self.x_cal, setter=self._set_x_cal)

#         # Outputs
#         self.register_real_variable("K", "Kalman gain", "output", getter=lambda: self.K)
#         self.register_real_variable("x_hat", "State estimate", "output", getter=lambda: self.x_hat)

#         # (Optional) parameter to set initial P
#         self.register_real_variable("P_init", "Initial covariance", "parameter",
#                                     getter=lambda: self._P_prev, setter=self._set_P_init)
...
#         denom = p_prev + sigma
#         if denom <= 0.0:
#             # basic guarding; you can choose a more sophisticated strategy
#             self.K = 0.0
#         else:
#             self.K = p_prev / denom
